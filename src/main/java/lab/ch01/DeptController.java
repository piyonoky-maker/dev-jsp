...
public class DeptController extends HttpServlet {
	...
	//부서 정보 등록하기
	//사용자로 부터 입력받으 ㄴ값을 전달 받아서 처리하기
	//Fribt-End: React -> JSON형식
	//jsp: 폼 전송 받음
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		log.info("doPost");
		
		//요청 본문이 JSON포맷일 떄 입려값 요청하기
		...
		PrintWriter out = resp.getWriter();
		//서블릿에서 오라클 연동하기 까지 처리함 - myBatis 외부 라이브러리 사용
		//out.print("<h1>"+result+"</h1>");		//등록성공이면 : 1 실패이면 : 0
		out.print(result);		//등록성공이면 : 1 실패이면 : 0
		out.flush();
	}
	...
	//부서정보 수정하기
	//update dept set dname=?, loc=? where where deptno = ?
	@Override
